  ...
  proc dispatch {request} {
    ...
    # router match
        ...
        if { $route == "" } {
          ...
          set ns_enter [find_ns_middlewares enter *]

          if {[string tolower $method] == "head" || [string tolower $method] == "options"} {
            foreach it $ns_enter {
              lassign $it md_method handler
              if {[string match -nocase $md_method $method]} {
                lassign $handler code_type code

                if {$code_type == "proc"} {
                  return [$code $request]
                } elseif {$code_type == "lambda"} {
                  return [apply $code $request]
                } else {
